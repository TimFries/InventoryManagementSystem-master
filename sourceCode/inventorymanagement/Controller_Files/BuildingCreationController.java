...
import DataEntities.EntityHandlers.BuildingInfoHandler;
...
public class BuildingCreationController implements Initializable {
        ...
        if (formCheck()) {
            ...
            if (checkFormPatterns()) {
                ...
                if (alertUser()) {
                    ...
                    BuildingInfoHandler.create(buildingData);
                    exitWindow();
                    
                } else {
                    System.out.println("User said no to final check");
                }
            } else {
    ...
    private void handleFieldFormat(KeyEvent ke) {
        ...
        if (ke.getSource().equals(zipcodeField)) {
            ...
            if (addressField.getText().trim().length() > 63) {
                ke.consume();
            }
    ...
    private boolean alertUser() {
        ...
        Alert alertPrompt = new Alert(AlertType.CONFIRMATION);
